...
public class ArithmeticView 
{
    ...
    private void observeModelAndUpdateControls() 
    {
        ...
        resultLabel.textProperty().bind(model.resultProperty().asString());
    }
    ...
    private void createAndLayoutControls() 
    {
        ...
        HBox operationsRow = new HBox(4);
        operationsRow.setAlignment(Pos.CENTER);

        // Create radio buttons for each size and add them to the toggle group
        for (Operation operation : Operation.values()) 
        {
            RadioButton radioButton = createRadioButton(operation, toggleGroup);
            operationsRow.getChildren().add(radioButton);
        }


        HBox resultRow = new HBox(5, new Label("Result:"), resultLabel);
        resultRow.setAlignment(Pos.CENTER);

       
        view.getChildren().addAll(xRow, yRow, operationsRow, resultRow);
    }
    ...
    private RadioButton createRadioButton(Operation operation, ToggleGroup toggleGroup) 
    {
        RadioButton radioButton = new RadioButton(operation.name());
        radioButton.setToggleGroup(toggleGroup);

        // Attach an event handler to handle the selection
        radioButton.setOnAction(event -> {          
            controller.updateSelctedOperation(operation);
        });
        return radioButton;
    }
}
...
enum Operation {
    ADDITION, SUBTRACTION, MUTLTIPLICATION, DVIVISION
}