...
public class PuppyTracker {
    ...
    void updatePuppyDetails(String name, int age, double weight, double length)
    {
        Dog dog = findByName(name);
        if (dog == null)
        {
    ...
    void printPuppyDetails(String name)
    {
        ...
        if (dog.age < dog.getAdultAge())
        {
            for (double i = dog.age + 1.0; i <= dog.getAdultAge(); i++) {
                ...
                System.out.println("Dog name: " + dog.name + "future age: " + dog.age + ", future weight: " + features.weight + 
